...
(function() {
    ...
    async function laiVaoTiem(nextState) {
        ...
        else { resetTrangThai("Không tìm thấy link siêu thị TGD_DON_BHO. Vui lòng kiểm tra lại quyền BI."); }
    }
    ...
    function hienThiCommanderDoanhThu(tong, chiTiet) {
        ...
        unsafeWindow.renderDailyDtht = function() {
            const raw = document.getElementById('inpRaw').value; const dt = parseFloat(document.getElementById('inpDT').value) || 0; const tg = parseFloat(document.getElementById('inpTG').value) || 0; const time = document.getElementById('inpTime').value; const mode = unsafeWindow.currentModeDtht; const masterTarget = unsafeWindow.masterTargetDtht;
            if(!raw) return;
            ...
            const exN = t => { let m = t.match(/[\d\.]+/); return m ? parseFloat(m[0].replace(/,/g, '')) : 0; };
            const exP = t => { let m = t.match(/[\d\.]+/g); return m ? (m.length >= 2 ? parseFloat(m[1].replace(/,/g, '')) : parseFloat(m[0].replace(/,/g, ''))) : 0; };
    ...
    function injectSKSTHTML() {
        ...
        overlay.innerHTML = `
            ...
            <div class="settings-panel" id="settingsPanel">
                ...
                <span style="font-size:13px; font-weight:900; color:#004a99;">👤 ĐỔI TÊN NHÂN SỰ</span>
        ...
        unsafeWindow.skst_openSettings = function() {
            ...
            // Xây dựng danh sách Nhóm Hàng có nút Sticker
        ...
        unsafeWindow.skst_saveSettings = function() {
            ...
            // Lưu lại tên và sticker nhóm hàng
        ...
        unsafeWindow.skst_closeSettings = () => { document.getElementById('settingsPanel').style.display = 'none'; document.getElementById('setOverlay').style.display = 'none'; }
        unsafeWindow.skst_closeDetail = () => { document.getElementById('dpDetail').style.display = 'none'; document.getElementById('ovDetail').style.display = 'none'; }
...
let r = `🚀 SỨC KHỎE TAM HÒA ĐẾN NGÀY ${toStkNum(new Date().getDate())}/${toStkNum(new Date().getMonth()+1)} 🚀\n🏥 1. TÌNH HÌNH CHUNG\n🔥 Doanh thu: ${toStkNum(dtHienTai)}/${toStkNum(targetMoi)} đạt ${lamTronTamHoa(phanTramMoi)}%\n⚡ Nhắc nhở: Còn ${toStkNum(days)} ngày, cần bứt phá ${lamTronTamHoa(canBucPha)}tr/ngày để về đích nhé team! 🏁\n💸 Góp ST: ${toStkNum(lamTronTamHoa(tgNum))}%\n📦 Nhóm hàng đạt/Tổng nhóm: ${toStkNum(actG.filter(g => g.pb >= 100).length)}/${toStkNum(actG.length)}\n`;
            ...
            r += `💸 Trả góp (TG):\n` + getTop(st, 'tg').map((s,i) => `${medals[i]} Top ${i+1}: ${s.name} ${lamTronTamHoa(s.tg)}%`).join("\n") + "\n📉 Cần lưu ý: " + getTop(st, 'tg', 3, true).map(s => `${s.name} ${lamTronTamHoa(s.tg)}%`).join(", ") + " Trả Góp Thấp team chú ý hành động cải thiện số.\n\n";
