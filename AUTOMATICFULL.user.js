...
(function() {
    ...
    // =========================================================================
    // PHÒNG 0: BỘ CÔNG CỤ LÕI & LUẬT LÀM TRÒN CHUẨN TAM HÒA
    // =========================================================================
    ...
    const lamTronTamHoa = (num) => {
        ...
        let result;
        if (absNum > 0 && absNum < 1.5) {
            result = 1;
        } else {
            result = Math.round(absNum);
        }
    ...
    function taoBangDieuKhienMenu() {
        ...
        const radDT = document.getElementById('radio-dt'), radSK = document.getElementById('radio-sk'), radV16 = document.getElementById('radio-v16');
        const btnStart = document.getElementById('btn-start-flow');
        const lblDT = document.getElementById('lbl-dt'), lblSK = document.getElementById('lbl-sk'), lblV16 = document.getElementById('lbl-v16');

        const updateRadios = () => {
            lblDT.style.opacity = radDT.checked ? '1' : '0.4';
            lblSK.style.opacity = radSK.checked ? '1' : '0.4';
            lblV16.style.opacity = radV16.checked ? '1' : '0.4';
            btnStart.disabled = false; btnStart.style.cursor = 'pointer'; btnStart.style.opacity = '1';
        };

        radDT.onchange = updateRadios; radSK.onchange = updateRadios; radV16.onchange = updateRadios;

        btnStart.onclick = () => {
            let task = radDT.checked ? 'DT_GO_HOME' : (radSK.checked ? 'SK_GO_HOME' : 'V16_GO_HOME');
    ...
    async function boNaoKiemTraTrangThai() {
        ...
        try {
            ...
            else if (currentState === 'V16_GO_HOME') { await laiVaoTiem('V16_FETCH'); }
            ...
            else if (currentState === 'V16_FETCH') { await xuLyV16_Main(); }
    ...
    // =========================================================================
    // XƯỞNG 1: SKST QUẢN TRỊ TAM HÒA (V16 GỘP)
    // =========================================================================
    async function xuLyV16_Main() {
        try {
            const cleanNum = (str) => {
                if (!str) return 0;
                let raw = str.toString().trim().split(/\s+/)[0].replace(/,/g, '');
                return parseFloat(raw) || 0;
            };
            const getCardPercent = (id) => {
                const card = document.getElementById(id);
                if(card) {
                    const matches = card.innerText.match(/[\d\.]+\%/g);
                    if (matches && matches.length > 1) return matches[1];
                    if (matches) return matches[0];
                }
                return "N/A";
            };
            const getKpiText = (chartId, label) => {
                const chart = document.getElementById(chartId);
                if (!chart) return "N/A";
                for (let r of chart.querySelectorAll('.row')) {
                    if (r.innerText.includes(label)) {
                        const valCol = r.querySelector('.font-weight-bold');
                        return valCol ? valCol.innerText.trim().split('\n')[0] : "N/A";
                    }
                }
                return "N/A";
            };
            const bungNhomChinhXac = async (tenNhom) => {
                return new Promise((resolve) => {
                    const rows = Array.from(document.querySelectorAll('#tbl-bcdtnh-data tbody tr'));
                    const targetRow = rows.find(tr => {
                        const cells = tr.querySelectorAll('td');
                        let rowName = cells.length > 1 ? cells[1].innerText.trim() : "";
                        return rowName === tenNhom || rowName === ("NNH " + tenNhom);
                    });

                    if (targetRow) {
                        const plusBtn = targetRow.querySelector('i.fa-plus');
                        if (!plusBtn || plusBtn.classList.contains('fa-minus')) {
                            return resolve(true);
                        }
                        plusBtn.click();
                        const observer = new MutationObserver((mutations, obs) => {
                            const newRows = document.querySelectorAll('#tbl-bcdtnh-data tbody tr');
                            if (newRows.length > rows.length) {
                                obs.disconnect();
                                resolve(true);
                            }
                        });
                        observer.observe(document.querySelector('#tbl-bcdtnh-data tbody'), { childList: true });
            ...
            await delay(3500); // Đợi Dashboard Load Xong

            const dtlkEl = document.querySelector('#revContentChart_20 .font-weight-bold.h6');
            const dtckEl = document.querySelector('#revContentChart_25 .font-weight-bold.h6');

            const soTong = {
                dtlk: cleanNum(dtlkEl ? dtlkEl.innerText : 0),
                dtck: cleanNum(dtckEl ? dtckEl.innerText : 0),
                dtqd: getKpiText('revContentChart_21', 'DTQĐ'),
                targetQd: getKpiText('revContentChart_21', 'Target (QĐ)'),
                pctDkht: getKpiText('revContentChart_21', '% HT Target Dự Kiến (QĐ)'),
                traGop: getCardPercent('revContentChart_23'),
                tlpvtc: getCardPercent('revContentChart_24')
            };

            soTong.chenhLech = lamTronTamHoa(soTong.dtlk - soTong.dtck);
            soTong.phanTram = soTong.dtck > 0 ? ((soTong.dtlk / soTong.dtck - 1) * 100).toFixed(2) : 0;

            const tabBtn = document.getElementById('tab-bcdtnh');
            if (tabBtn) tabBtn.click();
            await waitForElement('#tbl-bcdtnh-data tbody tr');
            await delay(1500);

            await bungNhomChinhXac("Điện thoại mới");
            await bungNhomChinhXac("Smartphone");

            let parentItems = [], smartItems = [], otherItems = [];
            let isInsideSmart = false;
            let donGiaTrungBinhTong = "0";

            document.querySelectorAll('#tbl-bcdtnh-data tbody tr').forEach(tr => {
                if (tr.style.display === 'none' || tr.querySelector('[colspan]')) return;
                const cells = tr.cells;
                if (!cells || cells.length < 11) return;

                let rawName = cells[1].innerText.trim();
                const tenNhom = rawName.replace("NNH ", "");

                if (tenNhom.toLowerCase() === 'tổng') {
                    donGiaTrungBinhTong = cells[8].innerText.trim();
                    return;
                }
                if (tenNhom === "" || tenNhom === "N/A") return;

                if (tenNhom === "Điện thoại mới") isInsideSmart = true;
                if (rawName.startsWith("NNH ") && tenNhom !== "Điện thoại mới") isInsideSmart = false;

                const sl = cells[2].innerText.trim();
                const dtqdRaw = cells[3].innerText.trim();
                const dtqdNum = cleanNum(dtqdRaw);
                let target = cells[4].innerText.trim();
                const pctTangGiam = cells[7].innerText.trim();
                const donGia = cells[8].innerText.trim();
                const dtTraGop = cells[9].innerText.trim();
                const tyTrongTG = cells[10].innerText.trim();

                let pctNum = parseFloat(pctTangGiam.replace('%', '').replace(',', ''));
                if (isNaN(pctNum)) pctNum = 0;

                let dtckNum = 0;
                if (pctNum !== -100) {
                    dtckNum = dtqdNum / (1 + (pctNum / 100));
                }

                if (dtqdNum === 0 && Math.round(dtckNum) === 0) return;

                let tienTangGiamNum = dtqdNum - dtckNum;
                let tienTangGiam = lamTronTamHoa(tienTangGiamNum);
                let dtckStr = pctNum === -100 ? "—" : formatMoney(Math.round(dtckNum));

                const vipGroups = ["Điện thoại mới", "Điện Thoại Di Động", "Smartphone"];
                if (isInsideSmart && !vipGroups.includes(tenNhom)) {
                    target = "—";
                }
                ...
                if (vipGroups.includes(tenNhom)) {
                    parentItems.push(itemData);
                } else if (isInsideSmart) {
                    smartItems.push(itemData);
                } else {
    ...
    function renderUIV16(soTong, parents, smart, other, donGiaTong) {
        ...
        overlay.id = 'report-v15';
        const now = new Date();
        const dateStr = now.getDate() + "/" + (now.getMonth() + 1);
        overlay.style.cssText = "position:fixed; top:0; left:0; width:100vw; height:100vh; background:#E2E8F0; z-index:999999; display:flex; flex-direction:column; font-family: 'Segoe UI', Arial, sans-serif; overflow-y:auto; overflow-x:hidden;";

        const sign = soTong.chenhLech >= 0 ? "▲" : "▼";
        const tColor = soTong.chenhLech >= 0 ? "#16A34A" : "#DC2626";

        let colorPct = "#0F172A";
        let valPct = parseFloat(soTong.pctDkht.replace('%', '').replace(',', ''));
        if (!isNaN(valPct)) {
            if (valPct < 80) colorPct = "#DC2626";
            else if (valPct < 100) colorPct = "#D97706";
            else colorPct = "#16A34A";
        }

        let txt = `🏥 BÁO CÁO SKST TAM HÒA ĐẾN ${dateStr}\n`;
        txt += `• Lũy Kế: ${formatMoney(soTong.dtlk)} / Cùng Kỳ: ${formatMoney(soTong.dtck)} (${sign} ${soTong.phanTram}% | ${soTong.chenhLech >= 0 ? '+' : ''}${formatMoney(soTong.chenhLech)} tr)\n`;
        txt += `• Quy Đổi: ${soTong.dtqd} / Target: ${soTong.targetQd} (Đạt: ${soTong.pctDkht})\n`;
        txt += `• Trả Góp: ${soTong.traGop} | TLPVTC: ${soTong.tlpvtc}\n`;
        txt += `• Đơn giá bình quân: ${donGiaTong}\n`;
        txt += `--------------------------------------------------\n`;

        const renderText = (items) => {
            items.forEach(x => {
                let statusIcon = x.valTien > 0 ? "🟢" : (x.valTien < 0 ? "🔴" : "⚪");
                let arrowIcon = x.valTien > 0 ? "▲" : (x.valTien < 0 ? "▼" : "—");
                let absTien = Math.abs(x.valTien);
                txt += `${statusIcon} ${x.tenNhom}: ${x.dtqdRaw} ${arrowIcon} ${x.pctTangGiam} (${formatMoney(absTien)}tr)\n`;
            });
        };
        ...
        const generateRow = (r, isChild = false) => {
            ...
            const indent = isChild ? "padding-left: 25px; color:#0F172A;" : "font-weight:900; color:#0F172A; background:#F1F5F9;";
            const prefix = isChild ? "↳ " : "";
            const statusIcon = r.valTien > 0 ? "🟢" : (r.valTien < 0 ? "🔴" : "⚪");
            const arrowIcon = r.valTien > 0 ? "▲" : (r.valTien < 0 ? "▼" : "");

            return `
                <tr style="border-bottom:1px solid #E2E8F0;">
                    <td style="padding:12px 10px; ${indent}">${statusIcon} ${prefix}${r.tenNhom}</td>
                    <td style="padding:12px 10px; text-align:right; font-weight:700; color:#0F172A;">${r.sl}</td>
                    <td style="padding:12px 10px; text-align:right; font-weight:900; color:#0F172A;">${r.dtqd}</td>
                    <td style="padding:12px 10px; text-align:right; font-weight:700; color:#0F172A;">${r.donGia}</td>
                    <td style="padding:12px 10px; text-align:right; font-weight:700; color:#0F172A;">${r.target}</td>
                    <td style="padding:12px 10px; text-align:right; font-weight:700; color:#0F172A; background:#F8FAFC; border-left:2px solid #CBD5E1;">${r.dtck}</td>
                    <td style="padding:12px 10px; text-align:right; font-weight:900; color:${colorTien};">${arrowIcon} ${r.pctTangGiam}</td>
                    <td style="padding:12px 10px; text-align:right; font-weight:900; color:${colorTien};">${r.valTien > 0 ? '+' : ''}${r.tienTangGiam}</td>
                    <td style="padding:12px 10px; text-align:right; font-weight:700; color:#0F172A; border-left:1px dashed #CBD5E1;">${r.dtTraGop}</td>
                    <td style="padding:12px 10px; text-align:right; font-weight:900; color:#0F172A;">${r.tyTrongTG}</td>
                </tr>`;
        };

        const tableHeader = `
            <tr>
                <th style="padding:12px 10px; text-align:left; border-right:1px solid #334155;">Ngành hàng</th>
                <th style="padding:12px 10px; text-align:right;">SL</th>
                <th style="padding:12px 10px; text-align:right;">DTQĐ</th>
                <th style="padding:12px 10px; text-align:right;">Đơn Giá</th>
                <th style="padding:12px 10px; text-align:right;">Target</th>
                <th style="padding:12px 10px; text-align:right; border-left:2px solid #94A3B8;">DTCK</th>
                <th style="padding:12px 10px; text-align:right;">% +/-</th>
                <th style="padding:12px 10px; text-align:right;">+/- Tiền</th>
                <th style="padding:12px 10px; text-align:right; border-left:1px dashed #94A3B8;">Trả Góp</th>
                <th style="padding:12px 10px; text-align:right;">Tỷ Trọng</th>
            </tr>`;

        let boxHtml = `
            <div id="capture-zone" style="background:#F8FAFC; width:100%; padding:20px; display:flex; flex-direction:column; margin-bottom: 20px;">
                <h1 style="text-align:center; color:#0F172A; text-transform:uppercase; font-size:26px; font-weight:900; margin-bottom:15px; letter-spacing: 1px;">SỨC KHỎE SIÊU THỊ TAM HÒA ĐẾN ${dateStr}</h1>
                <div style="display:grid; grid-template-columns: repeat(5, 1fr); gap:15px; margin-bottom:20px;">
                    <div style="background:#FFF; border:2px solid #CBD5E1; padding:15px; border-radius:8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); text-align:center;">
                        <div style="font-size:13px; color:#0F172A; font-weight:900; margin-bottom:8px; text-transform:uppercase;">DT THỰC / CÙNG KỲ</div>
                        <div style="font-size:26px; font-weight:900; color:#0F172A;">${formatMoney(soTong.dtlk)} <span style="font-size:18px; color:#0F172A;">/ ${formatMoney(soTong.dtck)}</span></div>
                        <div style="font-size:16px; font-weight:900; color:${tColor}; margin-top:8px;">${sign} ${soTong.phanTram}% (${soTong.chenhLech >= 0 ? '+' : ''}${formatMoney(soTong.chenhLech)}tr)</div>
                    </div>
                    <div style="background:#FFF; border:2px solid #CBD5E1; padding:15px; border-radius:8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); text-align:center;">
                        <div style="font-size:13px; color:#0F172A; font-weight:900; margin-bottom:8px; text-transform:uppercase;">DTQĐ / TARGET</div>
                        <div style="font-size:26px; font-weight:900; color:#0F172A;">${soTong.dtqd} <span style="font-size:18px; color:#0F172A;">/ ${soTong.targetQd}</span></div>
                        <div style="font-size:16px; font-weight:900; color:${colorPct}; margin-top:8px;">Đạt: ${soTong.pctDkht}</div>
                    </div>
                    <div style="background:#FFF; border:2px solid #CBD5E1; padding:15px; border-radius:8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); text-align:center;">
                        <div style="font-size:13px; color:#0F172A; font-weight:900; margin-bottom:8px; text-transform:uppercase;">TRẢ GÓP</div>
                        <div style="font-size:32px; font-weight:900; color:#0F172A; margin-top:12px;">${soTong.traGop}</div>
                    </div>
                    <div style="background:#FFF; border:2px solid #CBD5E1; padding:15px; border-radius:8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); text-align:center;">
                        <div style="font-size:13px; color:#0F172A; font-weight:900; margin-bottom:8px; text-transform:uppercase;">TLPVTC</div>
                        <div style="font-size:32px; font-weight:900; color:#0F172A; margin-top:12px;">${soTong.tlpvtc}</div>
                    </div>
                    <div style="background:#FFF; border:2px solid #CBD5E1; padding:15px; border-radius:8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); text-align:center;">
                        <div style="font-size:13px; color:#0F172A; font-weight:900; margin-bottom:8px; text-transform:uppercase;">ĐƠN GIÁ</div>
                        <div style="font-size:32px; font-weight:900; color:#0F172A; margin-top:12px;">${donGiaTong}</div>
                    </div>
                </div>

                <div style="background:#FFF; border:2px solid #0F172A; border-radius:8px; overflow:hidden; box-shadow: 0 4px 8px rgba(0,0,0,0.15); margin-bottom:20px;">
                    <table style="width:100%; border-collapse:collapse; font-size:15px;">
                        <thead style="background:#0F172A; color:#FFF;">
                            <tr><th colspan="10" style="padding:12px; font-weight:900; font-size:18px; text-align:left; letter-spacing:0.5px;">📱 ĐIỆN THOẠI MỚI (CHỦ LỰC)</th></tr>
                            ${tableHeader}
                        </thead>
                        <tbody>
        `;
    ...
    function hienThiCommanderDoanhThu(tong, chiTiet) {
            ...
            <div class="th-container" id="dthtMainContainer">
                ...
                <div id="outputArea" style="display:none;">
                    <div id="finalReportArea">
                        <div id="realtime-header" style="text-align: center; color: #fff; background: #004a99; padding: 10px 0; font-size: 16px; font-weight: 950; border-radius: 10px; margin-bottom: -1px; border: 2px solid #ffc107;">⏰ REALTIME --:--</div>
        ...
        unsafeWindow.renderDailyDtht = function() {
            ...
            if (datCount === tGroups && tGroups > 0) {
                boxDat.style.background = "#FFD700"; boxDat.style.borderColor = "#e67e22"; elDat.style.color = "#000";
            } else if (datCount >= (tGroups / 2)) {
                boxDat.style.background = "#e8f8f5"; boxDat.style.borderColor = "#2ecc71"; elDat.style.color = "#27ae60";
            } else {
                boxDat.style.background = "#fdedec"; boxDat.style.borderColor = "#e74c3c"; elDat.style.color = "#c0392b";
            }
    ...
    // =========================================================================
    // XƯỞNG 3: SỨC KHỎE NHÂN VIÊN (sknv) TỪ V9.1
    // =========================================================================
    ...
    function injectSKSTHTML() {
        ...
        overlay.innerHTML = `
            <div class="header-controls">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
                    <span style="font-weight: 900; color: #004a99; font-size: 13px;">📊 BẢNG SỨC KHỎE NHÂN VIÊN</span>
            ...
            <div class="settings-panel" id="settingsPanel">
                <h3 style="text-align: center; color: #004a99; font-weight: 900; margin: 0 0 15px 0;">⚙️ CÀI ĐẶT SỨC KHỎE NHÂN VIÊN</h3>
        ...
        unsafeWindow.skst_showStaffDetail = function(sid) {
            ...
            document.getElementById('stHeader').innerText = `SỨC KHỎE ĐẾN NGÀY ${daysPassed}/${new Date().getMonth()+1}`;
